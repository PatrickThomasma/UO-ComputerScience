...
def receive_packet(connection, max_buffer_size):
    ...
    write_to_file("output/received_by_router_6.txt",decoded_packet, None)
...
def start_server():
    ...
    while True:
        ...
        connection, address = soc.accept()
        #packet = receive_packet(connection, 5120)
...
def processing_thread(connection, ip, port, forwarding_table,forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    while True:
        ...
        if new_ttl >= 0:
            print("OUT:", payload)
            write_to_file("output/out_router_6.txt", str(payload), None)
