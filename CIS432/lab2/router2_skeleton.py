...
def receive_packet(connection, max_buffer_size):
    ...
    received_packet = connection.recv(1024)
    ...
    write_to_file("output/received_by_router_2.txt",decoded_packet, None)
...
def start_server():
    ...
    while True:
        ...
        try:
             Thread(target = processing_thread, args =(connection,ip,port,forwarding_table_with_range,forwarding_table, default_gateway_port)).start()
        except:
...
def processing_thread(connection, ip, port, forwarding_table_with_range, forwarding_table,default_gateway_port, max_buffer_size=5120):
    # 1. Connect to the appropriate sending ports (based on the network topology diagram).
    s_1 = create_socket("127.0.0.1", 8003)
    ...
    while True:
        ...
        payload = packet[2]
        ttl = packet[3]
        #print("Check out my destination", destinationIP)

        #print("this is my ttl", ttl)
        ...
        #print("This is my IP with range", forwarding_table_with_range)
        #print("Payload", payload)
        #print(destinationIP_int)
        ...
        for index, (start,end) in enumerate(forwarding_table_with_range, start = 0):
            #print("The end", end)
            if destinationIP_int in range(start,end + 1):
                newport = index
                break
        ...
        if newport is None:    
            ...
            sendport = 8003
        #print("Payload:", payload , "sendport", sendport)

        #print(sendport)
        #print("This is my destination IP", destinationIP_int)
        ...
        if sendport == 8003 and new_ttl > 0:
            ...
            print("OUT:",payload)
            write_to_file("output/out_router_2.txt",str(payload),None)
        elif new_ttl > 0:
            print("OUT:", payload)
            write_to_file("output/out_router_2.txt", str(payload) , None)
        else:
