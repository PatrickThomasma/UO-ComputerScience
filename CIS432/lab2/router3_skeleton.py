...
def receive_packet(connection, max_buffer_size):
    ...
    write_to_file("output/received_by_router_3.txt",decoded_packet, None)
...
def start_server():
    ...
    forwarding_table = read_csv('router_3_table.csv')
...
def processing_thread(connection, ip, port, forwarding_table,forwarding_table_with_range, default_gateway_port, max_buffer_size=5120):
    ...
    #s_1 = create_socket("127.0.0.1", 8004)
    #s_2 = create_socket("127.0.0.1", 8003)
    ...
    while True:
        ...
        if newport is None:
            sendport = default_gateway_port
        else:
            sendport = forwarding_table[newport][3]
        ...
        if new_ttl >= 0:
            print("OUT:", payload)
            write_to_file("output/out_router_3.txt", str(payload) , None)
