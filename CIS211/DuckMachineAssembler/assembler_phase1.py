...
def transform(lines: List[str]) -> List[str]:
    ...
    for lnum in range(len(lines)):
        ...
        if fields["kind"] == AsmSrcKind.FULL:
            ...
            new_mem_addr = resolved[mem_addr]
            fix_optional_fields(fields)
            pc_relative = new_mem_addr - address
            f = fields
            full = (f"{f['label']}   {f['opcode']}{f['predicate']} " +
                    f" {f['target']},r0,r15[{pc_relative}] #{mem_addr} " +
                    f" {f['comment']}")
            ...
            full = (f"{f['label']}   {'ADD'}{f['predicate']} " +
                    f" r15,r0,r15[{resolved[mem_addr]-address}] #{mem_addr} " +
                    f" {f['comment']}")
            transformed.append(full)
        else:
            transformed.append(line)
...
def fix_optional_fields(fields: Dict[str, str]):
    ...
    if fields['label'] == None:
        fields['label'] = ''
    elif fields['label'] != None:
        #fields.update([(fields["label"],'lab:')])
        fields['label'] += ':'
    if fields['predicate'] == None:
        fields['predicate'] = ''
    elif fields['predicate'] != None:
