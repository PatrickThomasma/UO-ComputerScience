...
class Appt:
  ...
  def overlaps(self, other: 'Appt') -> bool:
      ...
      if self.start > other.start or other.start < self.finish:
          return True

  def intersect(self, other: "Appt") -> "Appt":
      """"""
      assert self.overlaps(other)
      appt2 = Appt(max(self.start, other.start),(min(self.finish,other.finish)), f" | {self.desc} and {other.desc}")
      return appt2
...
class Agenda(list):
  ...
  def conflicts(self) -> 'Agenda':
      ...
      between this agenda and the other '''
      agenda = []
      self.sort()
      for first in range(len(self)):
          for second in range(1, len(self)):
              if self[first].finish > self[second].start:
                  agenda.append(Appt(self[second].start, self[first].finish, f'{self[first].desc and self[second].desc}'))
          break
      return agenda
