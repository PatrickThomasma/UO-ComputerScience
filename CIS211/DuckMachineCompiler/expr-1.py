...
class Neg(UnOp):
    ...
    def gen(self,context,target: str):
        label = context.get_const_symbol(self.left.value)
        context.add_line(f"    LOAD {target},{label}")
...
class Comparison(Control):
    ...
    def condjump(self, context, target: str, label: str, jump_cond: bool = True):
...
class If(Control):
    ...
    def gen(self, context, target: str):
        label = context.new_label("else")
        newlabel = context.new_label("fi")
        self.cond.condjump(context, target, label, jump_cond=False)
        self.thenpart.gen(context, target)
        context.add_line(f"JUMP {newlabel}")
        context.add_line(f"{label}:")
        self.elsepart.gen(context, target)
        context.add_line(f"{newlabel}:")
