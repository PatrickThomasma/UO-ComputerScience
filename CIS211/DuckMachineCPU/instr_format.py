...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        enword = 0
        enword = instr_field.insert(self.op.value, enword)
        enword = cond_field.insert(self.cond.value,enword)
        enword = reg_target_field.insert(self.reg_target, enword)
        enword = reg_src2_field.insert(self.reg_src2, enword)
        enword = reg_src1_field.insert(self.reg_src1,enword)
        enword = offset_field.insert(self.offset, enword)

        return enword

def decode(word: int) -> Instruction:
        """Decode a memory word (32 bit int) into a new Instruction"""
        new_instruction = Instruction(OpCode(instr_field.extract(word)), CondFlag(cond_field.extract(word)),
                          reg_target_field.extract(word), reg_src1_field.extract(word),
                          reg_src2_field.extract(word),offset_field.extract_signed(word))
