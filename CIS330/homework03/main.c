...
{
  int i, row_size,index;
  row_size = m + 1;
  *csr_col_ind = (unsigned int *)malloc(nnz * sizeof(unsigned int));// allocate size for index space
  *csr_vals = (double *)malloc(nnz * sizeof(double)); //allocate size of number
  *csr_row_ptr = (unsigned int *)malloc((m+1) * sizeof(unsigned int));

  
  for (int i = 0; i < nnz; i++) {
	  (*csr_col_ind)[i] = (unsigned int)col_ind[i];
  } 
  for (i = 0; i < nnz; i++)
  {
	  (*csr_vals)[i] = (double)val[i];
  }
  for (i = 0; i < m + 1; i++)
  {
	  (*csr_row_ptr)[i] = 0;
  }
  int *histo = (int *)malloc(m * sizeof(int)); //creating histogram
  memset(histo,0,sizeof(int)*m+1);

  for (i = 0; i < nnz; i++) {
	  histo[row_ind[i]]++; //populating histo
  }
  for (i = 0; i < nnz; i++) //histo implementation
  {
	  if (i == 0) {
		  (*csr_row_ptr)[i] = histo[i];
		  continue;
	  }
	  (*csr_row_ptr)[i] = (*csr_row_ptr)[i-1] + histo[i];
  }
  for (i = 0; i < m; i++) { //prefix sum implementation
	  if ((*csr_row_ptr)[histo[i]] > 0) {
		  (*csr_col_ind)[i] = (*csr_row_ptr)[histo[i]];
		  (*csr_vals)[i] = (*csr_row_ptr)[histo[i]];
	  }
          ...
          double* vector_x, double* res)
{
  ...
  for (i = 0; i < m + 1; i++)
  {
	  res[i] = 0.0; //algorithm from slides
	  for (j = csr_row_ptr[i]; j < csr_row_ptr[i + 1]; j++)
	  { //one of hte inputs works
